...
func TestCrossPlatformCoverageFeedGroupQueryRejectsUnknownConversationEnvelope(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			err := root.Execute()
			var typed *apperrors.Error
			wantReason := "chat_category_response_invalid"
			if tc.name == "missing pagination fact" {
				wantReason = "incomplete_result"
			}
			if !errors.As(err, &typed) || typed.Reason != wantReason {
				t.Fatalf("feed group invalid response error = %#v", err)
			}
			if tc.name == "missing collection" {
				if output.Len() != 0 {
				...
				if payload["complete"] != false || payload["notFoundCount"] != float64(0) || payload["unresolvedCount"] != float64(1) || payload["foundCount"] != float64(1) {
					t.Fatalf("unknown exhaustion must preserve positive membership and unresolved negatives: %s", output.String())
				}
			}
		})
	}
