...
var MessagesSend = shortcut.Shortcut{
	...
	Intent:      "当你希望用同一个入口选择 current-user、bot 或 webhook 身份发送消息时使用；命令会按身份校验目标、内容和凭据并路由到真实下层。current-user 支持文本/Markdown、已有 mediaId 图片、安全相对路径文件上传和幂等键；bot 支持群聊或批量单聊文本/Markdown；webhook 的目标由 token 所在群决定。不会把 user 文件能力伪装成 bot/webhook 等价能力。",
	Risk:        shortcut.RiskWrite,
	Flags: []shortcut.Flag{
		{Name: "identity", Type: shortcut.FlagString, Default: "user", Enum: []string{"user", "bot", "webhook"}, Desc: "发送身份；身份相关目标、内容和凭据不能为空，完整能力矩阵见命令说明"},
		...
		{Name: "user", Type: shortcut.FlagString, Desc: "单聊接收者 userId（user；自动解析 openDingTalkId）"},
		...
		{Name: "robot-code", Type: shortcut.FlagString, Desc: "机器人 Code（identity=bot 时使用）"},
		{Name: "webhook-token", Type: shortcut.FlagString, Desc: "自定义机器人 Webhook token（identity=webhook 时使用）"},
	...
	Constraints: []shortcut.Constraint{
		...
		{Kind: shortcut.ConstraintMutuallyExclusive, Flags: []string{"uuid", "idempotency-key"}},
		{
			...
			Description: "身份相关目标、内容和凭据不能为空，完整能力矩阵见命令说明",
		},
	},
	Tips: []string{
...
func messagesSendUserTarget(rt *shortcut.RuntimeContext) (group, openID string, err error) {
	...
	if err != nil {
		return "", "", fmt.Errorf("把 --user 解析为 openDingTalkId 失败: %w", err)
	}
	openID = findOpenDingTalkID(data)
	if openID == "" {
		return "", "", apperrors.NewValidation("无法把 --user 解析为 openDingTalkId；请改传 --open-dingtalk-id")
	}
	return "", openID, nil
}
