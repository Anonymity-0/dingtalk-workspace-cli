...
var MessagesSend = shortcut.Shortcut{
	...
	Flags: []shortcut.Flag{
		...
		{Name: "robot-code", Type: shortcut.FlagString, RequiredWhen: "identity=bot", Desc: "机器人 Code（identity=bot 时使用）；受发送身份能力矩阵约束"},
		{Name: "webhook-token", Type: shortcut.FlagString, RequiredWhen: "identity=webhook", Desc: "自定义机器人 Webhook token（identity=webhook 时使用）；受发送身份能力矩阵约束"},
