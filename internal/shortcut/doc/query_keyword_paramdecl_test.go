...
func TestCrossPlatformCoverageQueryToKeywordParamDecls(t *testing.T) {
	assertQueryKeywordParamDecl(t, "+search", Search)
	if len(shortcut.FromShortcut(Search).Schema.Parameters) == 0 {
		t.Fatal("+search Schema.Parameters empty after FromShortcut")
	}
}
