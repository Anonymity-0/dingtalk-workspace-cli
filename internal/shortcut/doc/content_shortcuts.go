...
var Fetch = shortcut.Shortcut{
	...
	Intent:      "当用户要按 node/URL 直接读取在线文字文档，或只知道唯一标题并希望一次完成解析和读取时使用；支持 outline/range/section/keyword/tags 局部内容用于精确编辑和评论；互联网公开文档（含密码保护）用 --password 提供访问密码，读历史版本用 --revision 指定版本号（0 表示初始版本）。",
	Risk:        shortcut.RiskRead,
	Safety:      contract.SafetySpec{Effect: "read", Risk: "low", Confirmation: "not_required", Idempotency: "idempotent"},
	Contract: docContract(
		"+fetch", "读取完整或局部文档内容，并按 detail 控制保真度",
		"当用户要按 node/URL 直接读取在线文字文档，或只知道唯一标题并希望一次完成解析和读取时使用；支持 outline/range/section/keyword/tags 局部内容用于精确编辑和评论；互联网公开文档（含密码保护）用 --password 提供访问密码，读历史版本用 --revision 指定版本号（0 表示初始版本）。",
		...
		contract.ParamDecl{Name: "revision", Property: "historyVersion"},
	),
	Flags: []shortcut.Flag{
		...
		{Name: "revision", Type: shortcut.FlagInt, Desc: "读取指定历史版本(版本号从 doc +version-list 获取, 0 表示初始版本, 需要文档编辑权限)；缺省读最新版"},
	},
	Tips: []string{`dws doc +fetch --node <DOC_ID>`, `dws doc +fetch --query "项目周报" --scope keyword --keyword "结论"`},
	Validate: func(rt *shortcut.RuntimeContext) error {
		if rt.Changed("revision") && rt.Int("revision") < 0 {
			return apperrors.NewValidation("--revision 必须为非负整数历史版本号（0 表示初始版本，从 doc +version-list 获取）")
		}
	...
	Execute: func(rt *shortcut.RuntimeContext) error {
		...
		if rt.Changed("revision") {
			params["historyVersion"] = rt.Int("revision")
		}
