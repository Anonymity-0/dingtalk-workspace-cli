...
func TestCrossPlatformCoverageQueryToKeywordParamDecls(t *testing.T) {
	assertQueryKeywordParamDecl(t, "+space-search", SpaceSearch)
	if len(shortcut.FromShortcut(SpaceSearch).Schema.Parameters) == 0 {
		t.Fatal("+space-search Schema.Parameters empty after FromShortcut")
	}
}
