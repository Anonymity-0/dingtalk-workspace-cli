...
- **MCP service disposition ledger** — migrates `schema_mcp_service_review.json` into `schema_mcp_service_review_ledger.go` so missing-service dispositions (`notify` → `out_of_surface`) and the pinned snapshot hash stay reviewed Go constants. Policy/tests read the ledger; runtime assembly still uses only `schema_mcp_metadata.json`.
