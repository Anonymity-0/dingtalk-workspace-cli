...
- filters 使用 `{"operator":"and|or","operands":[...]}`，字段引用使用 fieldId；完整操作符只读 [filter-sort](aitable/aitable-filter-sort.md)。
...
长 JSON 写到 cwd 内相对文件后使用 `--records-file ./records.json`。从真实返回的 `data.newRecordIds[]` 取 recordId，再用 `+record-query --record-ids` 回读。不要从输入顺序、名称或行号推断 ID。
...
- 记录历史：`+record-history-list`
- 记录分享：`+record-share-links` / `+record-share-url`
