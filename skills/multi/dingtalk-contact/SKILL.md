...
以下命令来自独立于 Runtime Schema 的公开 catalog。先运行 `dws shortcut list --service contact --format json` 读取完整契约，再用 `dws contact <shortcut> --help` 核对 flags；不要对 `+` 路径调用 `dws schema`。
