...
- 流式卡片明确“只创建/不更新/不填正文”时用原生 `message send-card`，群聊创建可传 ID/`--at-all`，只调用 create；同时提供正文或要求创建后写入/完成时才用 `+messages-send-card`，Runtime 把 create 返回前缀加到 `--content`。不得自行补正文；禁写占位符；仅 text。
