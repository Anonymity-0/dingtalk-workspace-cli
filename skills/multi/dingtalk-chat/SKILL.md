...
| 上传本地文件到会话空间且不发消息 | `dws chat file upload --conversation-id <cid> --file <路径>`；返回 `dentryId`/`spaceId`，不支持 URL |
...
- 发文件消息用 `+messages-send --file <路径>`；只存会话空间、不发消息才用 `chat file upload`，返回 `dentryId`/`spaceId`。
