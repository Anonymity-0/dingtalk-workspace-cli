---
name: dingtalk-event
description: 钉钉个人 IM 与 OA 审批事件长连接监听、订阅与消费，覆盖 @我、指定发送人、指定群、全部单聊/群消息、消息已读/撤回/表情回应、群生命周期，以及审批任务与审批实例事件。Use when 用户提到实时监听消息、被@消息、监听某人或某群、监听已读/撤回/表情回应、群成员加入/退出、群状态变化、监听待审批任务或审批实例、事件驱动 Agent。命令前缀：dws event。
---
