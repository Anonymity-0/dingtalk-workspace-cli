...
- `csv-put` 面向大批量 CSV 值/公式导入：输入 CSV 文本，写入指定起点；字段值以 `=` 开头时按公式解析，前加单引号时写成公式文本；不保留 dtype/format 协议，也不支持富格式
...
|------|---------------|----------|-----------|-------------|
| 公式（`=` 开头） | 支持 | 不支持 | 支持；前导单引号写公式文本 | 不作为公式协议；公式请用 `range update` 或 `csv-put` |
