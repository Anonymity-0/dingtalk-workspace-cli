...
> 本文件为 `dingtalk-misc` 内个人 IM 事件产品入口。命令前缀：`dws event`。命中后先读本页 Golden Route；子 reference 按需加载，不要预加载全部。
...
订阅创建/复用、单 bus、多 consumer、ready marker、扁平 NDJSON、超时/取消、部分失败回滚和退出清理全部复用现有 Runtime。低频 EventKey、群生命周期、Filter DSL、原始 envelope、复用 subscribe_id 等仍由 `event consume` 承担。
