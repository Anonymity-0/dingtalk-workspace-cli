...
> 本文件为 `dingtalk-misc` 内个人 IM 与 OA 审批事件产品入口。命令前缀：`dws event`。命中后先读本页 Golden Route；子 reference 按需加载，不要预加载全部。
...
| 群改名、成员进退、群解散 | 读取 [EventKey 索引](./event-im-keys.md)，使用精确 `event consume` EventKey |
| OA 审批任务或实例事件 | 读取 [OA 事件参考](./event-oa.md)，使用精确 `event consume` EventKey |
...
六个 OA EventKey 及其输出字段见 [OA 事件参考](./event-oa.md)。
...
- 事件只负责监听；需要回复时按 [输出与 Chat 交接](./event-im-output.md) 把真实 `conversation_id` 或 `sender_open_dingtalk_id` 交给 `dws chat +messages-send`，不要从显示名猜 ID。
...
- OA 扁平事件提供审批实例、任务和状态字段；字段差异、原始回退条件及与 OA 命令的稳定 ID 交接以 [OA 事件参考](./event-oa.md) 为准。
...
- 认证、profile、订阅保护状态和 bus 排障按失败类型读取 [订阅运维](./event-im-operations.md)，不要在正常路径预加载完整运维手册。
...
|---|---|---|
| 任务索引 | [event-im.md](./event-im.md) | 还不能判断应该加载哪一个子 reference |
| EventKey、目标规则与底层 consume | [event-im-keys.md](./event-im-keys.md) | 群生命周期、显式 EventKey 或多事件组合 |
| ready、bounded consume 与退出清理 | [event-im-lifecycle.md](./event-im-lifecycle.md) | 启动/托管/关闭 consumer |
| 扁平字段与事件到 Chat 交接 | [event-im-output.md](./event-im-output.md) | 解析事件或自动回复 |
| Filter、status/stop、重试与排障 | [event-im-operations.md](./event-im-operations.md) | 订阅控制或失败恢复 |
| OA 审批事件 | [event-oa.md](./event-oa.md) | 选择六个 OA EventKey、组合消费或解析审批字段 |
