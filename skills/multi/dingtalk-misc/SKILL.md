---
name: dingtalk-misc
description: 长尾产品集合技能，覆盖低频钉钉产品：OA审批/考勤/直播/DING紧急消息/开放平台应用管理/Agoal目标管理/日志日报周报/电子表格/开放平台文档搜索/文档内嵌白板/DWS技能市场安装/个人 IM 事件与 OA 审批事件/组织大脑Hrbrain/原生Markdown/PAT行为授权/多组织profile。Use when 用户提到上述任一产品，或审批/打卡/排班/OKR/日报周报/单元格读写/白板节点读写/搜索安装技能/开发者后台应用/实时监听消息/被@消息/监听某人或某群/监听已读撤回表情回应/监听待审批任务或审批实例/群成员加入/群成员退出/群状态变化/事件驱动 Agent/人才池/员工档案/职业历程/绩效/原生.md文件/PAT授权/切换组织/跨组织/profile 等相关操作。命中后由本 skill 的「产品索引表」定位具体子产品和命令前缀，再按对应子产品说明执行。
metadata:
  cli_version: ">=0.2.14"
  category: product
  requires:
...
| 个人 IM/OA 审批事件 / 实时监听 / 被@消息 / 监听某人某群 / 已读撤回表情回应 / 群成员加入退出 / 监听待审批任务或审批实例 / 事件驱动 Agent | 个人 IM 与 OA 审批事件长连接监听、订阅与消费 | `dws event` | [event.md](references/event.md) |
...
- 个人 IM 或 OA 审批事件命中后先读 [event.md](references/event.md)，再按需加载 `event-im*.md` 或 `event-oa.md`；查询或操作审批走 [oa.md](references/oa.md)，实时监听审批状态变化走 event；不要与开放平台应用事件 [dev/event.md](references/dev/event.md) 混淆。
