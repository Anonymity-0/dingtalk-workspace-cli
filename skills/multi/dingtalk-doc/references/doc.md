...
只读某个历史版本的内容而不回滚时，用 `dws doc +fetch --node <DOC_ID> --revision <N>`（版本号同样来自 `+version-list`，`0` 表示初始版本，需要文档编辑权限）；不要为了看历史内容而执行回滚。互联网公开文档（含密码保护）的读取见 [doc-read.md](doc/doc-read.md) 的 `--password`。
