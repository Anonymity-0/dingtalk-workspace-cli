...
- `chat group-role` 系列命令用于管理群的自定义身份标签：`list` 查列表，`add` 创建，`update` 改名，`remove` 删除；`set-user` 用 `--role-id` 覆盖某人的群身份，`remove-user` 仅移除指定身份，`query-user` 查询某人当前身份；用户用 `--user <userId>`
