...
不支持转换的格式（html/pdf/zip/无扩展名等所有白名单外格式）不报错：CLI 自动改走文件上传链路，以原文件形式存入 `--folder`/`--workspace` 指定的文档空间/知识库位置，stderr 会输出改道提示。若用户要的是"可编辑的在线文档"，先把内容转换为 md，再用 md 文件执行 `doc import`；若目标是钉盘，直接用 `dws drive upload`。
