...
| Recipe | 行动指南（固定路线） |
|--------|-------------------|
| query-group-chat | `dws chat +chat-messages --chat-query "<群名>" --time "<yyyy-MM-dd HH:mm:ss>" --page-all --output ./exports/messages.json --format json`；只接受唯一解析结果，并检查 `complete/hasMore/failures`。 |
| query-private-chat | `dws chat +chat-messages --user-query "<姓名>" --time "<yyyy-MM-dd HH:mm:ss>" --page-all --format json`；零命中/多候选停止消歧，其余完整性规则同 query-group-chat。 |
| escalate-ding | 三级升级：<br>1. `ding message send --robot-code <robotCode> --type app --users <userId> --content "<内容>"`（必填项见 [ding.md](../products/ding.md)）<br>2. `dws chat +messages-send --as user --group <openConversationId> --text "<内容>" --yes --format json` 群里提醒；执行 `--yes` 前先完成收件人、内容与影响确认<br>3. `todo task create --title "<标题>" --executors <userId> --priority 40` 建紧急待办<br>所有稳定 ID 来自同一 profile 的真实返回，不选搜索第一项。 |
| send-by-bot | 单群用 `--group`；多群用 `dws chat +messages-send --as bot --robot-code <robotCode> --groups <cid1,cid2> --title "<标题>" --text "<内容>" --yes --format json`。Runtime 去重并返回 `im.batch-write.v1` 逐目标 ledger，未知投递状态时停止重发。 |
| forward-message | 1. `dws chat +chat-messages --chat-query "<源群名>" --format json` 取得唯一源会话与真实 `openMessageId`<br>2. 从真实返回或唯一解析取得目标 `openConversationId`<br>3. `dws chat +messages-forward --src-conversation-id <srcCid> --msg-id <openMessageId> --dest-conversation-id <destCid> --yes --format json`；确认后执行并保留幂等键。若用户只是要转述内容而非转发原消息，改走 `+dm`/`+send-to-group`。 |
