...
- **实时个人事件例外**：普通 IM 消息、reaction、已读和撤回默认走 `dws event +listen-im ...`；OA 审批、VoIP 通话邀请、群生命周期、明确的原始 EventKey、Filter DSL、subscribe_id 或原始 envelope 使用 `dws event consume ... --flatten`。不要写脚本轮询消息历史、审批列表或通话记录
...
| `event`           | 个人 IM/OA/VoIP 事件：监听消息、群生命周期、审批任务、审批实例与通话邀请，NDJSON 输出（实时驱动 Agent）| [event.md](./references/products/event.md)                     |
