...
| `dws chat emotion favorite` | Add a media ID or a local image (jpg/jpeg/png/gif/webp/bmp, ≤10MB) to the current user's personal favorite emotions. | When the agent needs to save an available mediaId or a local image file as a reusable personal emotion; local images are uploaded through im/upload_media first, optionally preserving source message context. |
