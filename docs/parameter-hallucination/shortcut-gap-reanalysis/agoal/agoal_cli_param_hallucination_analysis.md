...
冻结基线为 `origin/main@5f906bf8b61bc54a75f6c2b46219a1deb682b600`。本轮覆盖 5 个此前未进入正式参数兜底范围的公开 Shortcut；识别出 2 个可直接自动兜底、3 个只能部分兜底、0 个只应增加保护的命令，其中高风险写入或敏感读取 0 个。
...
候选新增 17 条 PreParse 验证 fixture，覆盖 alias/canonical、block 与 ambiguous 三类路径。candidate-only 完整命令模板位于 `internal/app/param_alias_payload_equivalence_test.go`，正式候选未启用时不会扩大既有测试范围。
