...
候选新增 24 条 PreParse 验证 fixture，覆盖 alias/canonical、block 与 ambiguous 三类路径。candidate-only 完整命令模板位于 `internal/app/param_alias_payload_equivalence_test.go`，其中 `devapp +robot-config` 显式携带 canonical `--skills`，正式候选未启用时不会扩大既有测试范围。
