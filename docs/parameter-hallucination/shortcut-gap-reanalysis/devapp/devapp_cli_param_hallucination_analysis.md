...
冻结基线为 `origin/main@5f906bf8b61bc54a75f6c2b46219a1deb682b600`。本轮覆盖 6 个此前未进入正式参数兜底范围的公开 Shortcut；识别出 0 个可直接自动兜底、6 个只能部分兜底、0 个只应增加保护的命令，其中高风险写入或敏感读取 6 个。
...
| 2 | `devapp +robot-config` | `unified-app-id,name,brief,desc,icon-media-id,outgoing-url,event-callback-url,mode,skills` | 两个回调 URL、机器人名称、简介/描述、mediaId 角色并存 | 复用必要 concept；仅角色完整 URL 别名；generic callback-url 歧义 | 部分兜底 | 高 |
...
候选新增 22 条 PreParse 验证 fixture，覆盖 alias/canonical、block 与 ambiguous 三类路径。candidate-only 完整命令模板位于 `internal/app/param_alias_payload_equivalence_test.go`，正式候选未启用时不会扩大既有测试范围。
...
- `devapp +robot-config`：两个回调 URL、机器人名称、简介/描述、mediaId 角色并存。候选只处理“值不变且角色明确”的别名；其余通过 block/ambiguous 停止，不做查找、转换、枚举翻译或单复数扩展。
