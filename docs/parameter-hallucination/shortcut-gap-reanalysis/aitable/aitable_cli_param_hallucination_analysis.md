...
候选新增 46 条 PreParse 验证 fixture，覆盖 alias/canonical、block 与 ambiguous 三类路径。candidate-only 完整命令模板位于 `internal/app/param_alias_payload_equivalence_test.go`，正式候选未启用时不会扩大既有测试范围。
